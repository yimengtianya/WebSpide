 ...
 * @version V1.0
 */
public class C5i5j extends SpideEntryBase {
	...
	protected HashSet<?> setLinks(HtmlPage page, int pageNum) {
		HashSet<String> urlsList = linksStrings(page, "http://\\w+\\.5i5j\\.com/exchange/\\d+");
	...
	protected void parsePage(HtmlPage Mainpage, Object linkItem, int pageNum) {
		...
		html = html.replaceAll(".*您正在查看", " ");
		html = html.replaceAll("房源描述.*小区价格走势", " ");
		html = html.replaceAll("点击查看全部.*", " ");
		//
	...
	private void loadRegexRule() {
		// 站内标识
		regexTable.set("web_in_uid", new CRegex("信息编号:(\\d+)", 1));
		// 发布时间
		regexTable.set("release_time", CRegexLib.dateTimeRegex("", ""), CRegexLib.dateTimeOffsetRegex("", ""));
		// 总价
		regexTable.set("price", new CRegex("售价:([0-9.]+)\\s*万元", 1));
		// 单价
		regexTable.set("price_unit", new CRegex("单价:([0-9.]+)\\s*元", 1));
		// 产权
		...
		// 楼盘名称
		regexTable.set("build_name", new CRegex("楼盘名称:([^\\(\\[]*)[\\s\\(\\[]", 1));
		// 使用面积
		regexTable.set("usage_area", new CRegex("面\\s*积:([0-9.]+)平米", 1), new CRegex("([0-9.]+)㎡", 1));
		// 建筑面积
		regexTable.set("build_area", new CRegex("面\\s*积:([0-9.]+)平米", 1), new CRegex("([0-9.]+)㎡", 1));
		// 年代
		...
		// 楼层
		regexTable.set("build_layer", new CRegex("楼层:(\\d+)/\\d+楼", 1), new CRegex("楼层:(\\w+)层", 1));
		// 总楼层
		regexTable.set("build_max_layer", new CRegex("楼层:\\d+/(\\d+)楼", 1), new CRegex("楼层:.*/(\\w+)层", 1), new CRegex("共(\\w+)层", 1));
		// 地址-小区
		regexTable.set("address_city", new CRegex("小区名?称?[:\\s]*([\u0000-\uffff]*?)\\s", 1), new CRegex("\\s*([\u0000-\uffff]*?)小区", 1));
		// 地址
		regexTable.set("address", CRegexLib.addressRegex("所在版块:", ""), CRegexLib.addressRegex("", ""));
		// 开发商
