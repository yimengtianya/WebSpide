 ...
 * @version V1.0
 */
public class Cfocus extends SpideEntryBase {
	...
	protected HashSet<?> setLinks(HtmlPage page, int pageNum) {
		HashSet<String> urlsList = linksStrings(page, "http://\\w+\\.esf\\.focus\\.cn/view/\\d+\\.html.*", "http://\\w+\\.mytophome\\.com/prop/view/\\d+\\.html.*");
	...
	protected void parsePage(HtmlPage Mainpage, Object linkItem, int pageNum) {
		...
		html = html.replaceAll(".*房源详情", " ");
		html = html.replaceAll("您可能感兴趣的房源.*", " ");
		html = html.replaceAll("该小区的其他房源.*", " ");
