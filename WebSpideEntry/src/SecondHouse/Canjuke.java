 ...
 * @version V1.0
 */
public class Canjuke extends SpideEntryBase {
	...
	protected HashSet<?> setLinks(HtmlPage page, int pageNum) {
		HashSet<String> urlsList = linksStrings(page, "http://\\w+\\.anjuke\\.com/prop/view/\\w+\\?.*");
	...
	protected void parsePage(HtmlPage Mainpage, Object linkItem, int pageNum) {
		...
		html = html.replaceAll(".*新房源通知", " ");
		html = html.replaceAll("房价走势.*", " ");
		html = html.replaceAll("\\(租\\s*套\\s*售\\s*套\\)", " ");
