 ...
 * @version V1.0
 */
public class Ccentanet extends SpideEntryBase {
	...
	protected HashSet<?> setLinks(HtmlPage page, int pageNum) {
		HashSet<String> urlsList = linksStrings(page, "http://\\w+\\.centanet\\.com/ershoufang/[-\\w]+\\.html");
	...
	protected void parsePage(HtmlPage Mainpage, Object linkItem, int pageNum) {
		...
		html = html.replaceAll(".*分享", " ");
		html = html.replaceAll(".*首页", " ");
		html = html.replaceAll("所在分行.*", " ");
	...
	private void loadRegexRule() {
		...
		// 总价
		regexTable.set("price", new CRegex("([0-9.]+)\\s*万", 1));
		// 单价
		regexTable.set("price_unit", new CRegex("单\\s*价[:\\s]*([0-9.]+)\\s*元", 1));
		// 产权
		regexTable.set("house_property", new CRegex("(\\d+年)产权", 1));
		// 户型
		regexTable.set("house_style", CRegexLib.houseStyleRegex("", ""), new CRegex("户型:(\\w*)\\s", 1));
		// 结构
		...
		// 年代
		regexTable.set("build_time_year", new CRegex("([0-9]*)年建", 1), new CRegex("建?筑?年\\s*代[:\\s]*([0-9]*)", 1));
		// 朝向
		...
		// 总楼层
		regexTable.set("build_max_layer", new CRegex("楼层[:\\s]*\\d+/(\\d+)楼", 1), new CRegex("楼层[:\\s]*.*/(\\w+)层", 1), new CRegex("共(\\w+)层", 1));
		// 地址-小区
		regexTable.set("address_city", new CRegex("小区名?称?[:\\s]*([\u0000-\uffff]*?)\\s", 1));
		// 地址
		regexTable.set("address", CRegexLib.addressRegex("地\\s*址[:\\s]*\\s*", ""), CRegexLib.addressRegex("", ""));
		// 开发商
		...
		// 按揭首付
		regexTable.set("mortgage_down_payment", new CRegex("参?考?首\\s*付[:\\s]*([0-9.]*)", 1));
		// 月供
