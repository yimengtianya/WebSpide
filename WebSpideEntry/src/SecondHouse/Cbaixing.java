 ...
 * @version V1.0
 */
public class Cbaixing extends SpideEntryBase {
	...
	protected HashSet<?> setLinks(HtmlPage page, int pageNum) {
		HashSet<String> urlsList = linksStrings(page, "http://\\w+\\.baixing\\.com/ershoufang/\\w+\\.html");
	...
	protected void parsePage(HtmlPage Mainpage, Object linkItem, int pageNum) {
		...
		html = html.replaceAll(".*免费发送至微信", " ");
		html = html.replaceAll("你可能会喜欢.*", " ");
		html = html.replaceAll("赞助.*", " ");
		//
	...
	private void loadRegexRule() {
		// 站内标识
		regexTable.set("web_in_uid", new CRegex("信息编号[:\\s]*(\\d+)", 1));
		// 发布时间
		regexTable.set("release_time", CRegexLib.dateTimeRegex("", ""), CRegexLib.dateTimeOffsetRegex("", ""));
		// 总价
		regexTable.set("price", new CRegex("价格[:\\s]*([0-9.]+)万元", 1));
		// 单价
		regexTable.set("price_unit", new CRegex("\\(([0-9.]+)元/㎡\\)", 1));
		// 产权
		...
		// 使用面积
		regexTable.set("usage_area", new CRegex("面积[:\\s]*([0-9.]+)平米", 1), new CRegex("([0-9.]+)㎡", 1));
		// 建筑面积
		regexTable.set("build_area", new CRegex("面积[:\\s]*([0-9.]+)平米", 1), new CRegex("([0-9.]+)㎡", 1));
		// 年代
		regexTable.set("build_time_year", new CRegex("([0-9]*)年建", 1), new CRegex("建?筑?年代[:\\s]*([0-9]*)", 1));
		// 朝向
		regexTable.set("build_face", CRegexLib.houseFaceRegex("房?间?朝向[:\\s]*", ""), CRegexLib.houseFaceRegex("朝\\s*", ""));
		// 楼层
		regexTable.set("build_layer", new CRegex("楼层[:\\s]*(\\d+)/\\d+楼", 1), new CRegex("楼层[:\\s]*(\\w+)层", 1));
		// 总楼层
		regexTable.set("build_max_layer", new CRegex("楼层[:\\s]*\\d+/(\\d+)楼", 1), new CRegex("楼层[:\\s]*.*层/(\\w+)层", 1));
		// 地址-小区
		regexTable.set("address_city", new CRegex("小区名?称?[:\\s]*([\u0000-\uffff]*?)\\s", 1), new CRegex("\\s*([\u0000-\uffff]*?)小区", 1));
		// 地址
		regexTable.set("address", CRegexLib.addressRegex("地处", ""), CRegexLib.addressRegex("临近", ""), CRegexLib.addressRegex("详情介绍", "地图及交通"), CRegexLib.addressRegex("", ""));
		// 开发商
		regexTable.set("develop_company", new CRegex("开\\s*发\\s*商[:\\s]*(\\w+)\\s", 1));
		// 物业费
		regexTable.set("property_costs", new CRegex("物\\s*业\\s*费[:\\s]*([0-9.]*)元", 1));
		// 物业公司
		regexTable.set("property_company", new CRegex("物业公司[:\\s]*([\u0000-\uffff]*?公司)\\s", 1));
		// 按揭首付
