...
public class Chomelink extends SpideEntryBase {
	...
	protected HashSet<?> setLinks(HtmlPage page, int pageNum) {
		HashSet<String> urlsList = linksStrings(page, "http://\\w+\\.lianjia\\.com(\\.cn)?/ershoufang/\\w+\\.html");
	...
	private void loadRegexRule() {
		...
		// 总价
		regexTable.set("price", new CRegex("售价:?\\s*([0-9.]+)\\s*万", 1), new CRegex("\\s*([0-9.]+)\\s*万\\s*单价", 1), new CRegex("总\\s*价[:\\s]*([0-9.]+)\\s*万", 1));
		// 单价
