 ...
 * @version V1.0
 */
public class C58 extends SpideEntryBase {
	...
	protected HashSet<?> setLinks(HtmlPage page, int pageNum) {
		HashSet<String> urlsList = linksStrings(page, "http://\\w+\\.58\\.com/ershoufang/\\w+\\.shtml.*");
	...
	protected void parsePage(HtmlPage Mainpage, Object linkItem, int pageNum) {
		...
		explorer.getExplorer().getOptions().setJavaScriptEnabled(true);
		...
		html = html.replaceAll(".*我要报案", " ");
		html = html.replaceAll("小区成交记录", " ");
		html = html.replaceAll("配套设施.*小区简介", " ");
		html = html.replaceAll("进入经纪人店铺.*房源描述", " ");
		html = html.replaceAll("同价位你可能感兴趣的房源.*房源描述", " ");
		html = html.replaceAll("查看更多.*", " ");
	...
	private void loadRegexRule() {
		...
		// 单价
		regexTable.set("price_unit", new CRegex("单价[:\\s]*([0-9.]+)\\s*元", 1), new CRegex("\\(([0-9.]+)\\s*元/㎡\\)", 1));
		// 产权
		...
		// 使用面积
		regexTable.set("usage_area", new CRegex("([0-9.]+)㎡", 1));
		// 建筑面积
		...
		// 地址
		regexTable.set("address", CRegexLib.addressRegex("地\\s*址[:\\s]*", ""), CRegexLib.addressRegex("位于[:\\s]*", ""), CRegexLib.addressRegex("", ""));
		// 开发商
